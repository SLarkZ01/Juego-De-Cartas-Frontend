...
export default function LobbyInfo({ partidaEstado, jugadoresLobbyCount, puedesIniciar, onIniciar }: Props) {
  return (
      ...
      {partidaEstado === 'ESPERANDO' && (
        <div className="space-y-3">
          ...
          {puedesIniciar && jugadoresLobbyCount >= 2 && (
            <Button onClick={onIniciar} className="w-full bg-green-600 hover:bg-green-700">🎮 Iniciar Partida</Button>
          )}
        </div>
      )}
    </div>
